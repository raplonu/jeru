...
The roadmap lives at `~/project/<name>/ROADMAP.md`. `jeru roadmap` (no subcommand) parses the file and prints each section with a count of open (`[ ]`) vs. completed (`[x]`) items so you can see progress at a glance.

When `jeru claude init` generates a `CLAUDE.md`, it automatically includes a roadmap directive if `ROADMAP.md` exists, instructing Claude to read the roadmap at the start of each session and mark items complete as work progresses.
